  ...
  <main><div class="container">
    ...
    <div class="card col-span-1">
      <div class="card-header" style="justify-content:space-between; width:100%;">
        ...
        <form method="POST" action="/api/gemini/clear" style="margin:0;" onsubmit="return confirm('消去しますか？');">
          <button type="submit" style="font-size:12px; border:none; background:none; color:var(--mut); text-decoration:underline; cursor:pointer;">クリア</button>
        </form>
      </div>
      ${!hasGeminiKey ? html`<div style="background:#fee2e2; color:#ef4444; padding:8px; border-radius:8px; font-size:12px; margin-bottom:8px; text-align:center; font-weight:bold;">⚠️ Gemini APIキーが未設定です</div>` : ''}
      <div id="chat-history" class="list-area" style="background:var(--bg); padding:12px; border-radius:8px; border:1px solid var(--brd); margin-bottom:8px;">${chatHist.length===0?html`<div class="chat-msg ai-msg">こんにちは！画像を添付しての相談も可能です。</div>`:''}${chatHist.map(c => html`<div class="chat-msg ${c.role==='user'?'user-msg':'ai-msg'}">${c.message}</div>`)}</div>
      <div id="image-preview-container" style="display:none; margin-bottom:8px; position:relative; width:fit-content;"><img id="image-preview" style="max-height:80px; border-radius:8px; border:1px solid var(--brd);"><div id="clear-image" style="position:absolute; top:-5px; right:-5px; background:#ef4444; color:white; border-radius:50%; width:20px; height:20px; display:flex; align-items:center; justify-content:center; font-size:12px; cursor:pointer;">×</div></div>
      <form id="gemini-form" class="form-row" style="align-items:center; margin:0;"><label for="chat-image-input" style="font-size:24px; cursor:pointer; margin-right:5px;">📷</label><input type="file" id="chat-image-input" accept="image/*" style="display:none;" capture="environment"><input type="text" id="gemini-input" placeholder="メッセージ..." required autocomplete="off"><button type="submit" id="gemini-submit">▶</button></form>
    </div>
  ...
  <script>
    ...
    async function syncTimeAPI(lat, lng) {
      ...
      try {
        ...
        const n = new Date();
        document.getElementById('koyomi-display').innerHTML = '⌚ ローカル時計 | 西暦' + n.getFullYear() + '年 / 旧暦: ' + old[n.getMonth()];
      }
    }
